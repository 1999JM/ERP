...
public class OrderSheetController {
    ...
    public @ResponseBody ResponseEntity updateOrderSheets(@RequestBody Map<String,Object> osData, HttpServletRequest request) {
        ...
        String errorMessage = "";
        OrderSheetAddmDto orderSheetAddmDto = null;    //주문서 정보와
        List<OrderSheetDetailAddmDto> orderSheetDetailAddmDtos = new ArrayList<OrderSheetDetailAddmDto>(); //주문서 디테일 정보

        for (Map.Entry<String, Object> entry : osData.entrySet()) {
            Object value = entry.getValue();

            if(entry.getKey().equals("osInfo")) {//주문서의 정보가 담겨져 있는 데이터
        ...
        }else if(String.valueOf(orderSheetAddmDto.getDivisionStatus()) == "출하대기"){
            if(orderSheetAddmDto.getOsReceiptDate() == null){
                errorMessage ="출고 예정일을 선택해 주세요.";
                return new ResponseEntity<>(errorMessage,HttpStatus.BAD_REQUEST);
            }else if(dateTime.isAfter(orderSheetAddmDto.getOsReceiptDate())){   //현재일 보다 입고 예정일이 과거일수는 없습니다.
                errorMessage ="출고 예정일을 다시 선택해 주세요";
                return new ResponseEntity<>(errorMessage,HttpStatus.BAD_REQUEST);
            }
        }
